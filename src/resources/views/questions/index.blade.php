                ...
                <div class="flex items-center justify-between pt-5 border-t border-slate-50">
                    <div class="flex gap-4 text-sm font-medium text-slate-500">
                        <form action="/likeQuestion/{{$question->id}}" method="POST">
                            ...
                            <button type="submit" class="group flex items-center space-x-2 transition-colors {{$question->likes->contains( 'user_id', Auth::id()) ? 'text-red-600' : 'text-slate-400'}}  hover:text-red-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 transition-transform group-hover:scale-110 {{$question->likes->contains('user_id', Auth::id())? 'fill-current' : 'fill-none' }} " viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                </svg>
                                <span class="text-sm font-bold">{{$question->likes->count()}}</span>
                            </button>
                        </form>
                        ...
                        <form action="/showQuestion/{{$question->id}}" method="GET">
                            ...
                            <button type="submit" class="group flex items-center space-x-2 text-slate-400 hover:text-indigo-600 transition-colors">
                                ...
                                <span class="text-sm font-bold">{{$question->responses->count()}}</span>
                            </button>
                        </form>

                        <form action="/addFavorite/{{$question->id}}" method="POST">
                            @csrf
                            <!-- <input type="hidden" name="_token" value="VOTRE_TOKEN_ICI"> -->
                            <button type="submit" class="group transition-colors {{$question->favorites->contains('user_id', Auth::id()) ? 'text-yellow-500' : 'text-slate-400' }}  hover:text-yellow-500">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 transition-transform group-hover:scale-110 {{$question->favorites->contains('user_id', Auth::id()) ? 'fill-current' : 'fill-none' }} " viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <form action="/delete/{{$question->id}}" method="POST">
                            <!-- Utiliser DELETE rend ton code explicite. Quand un autre développeur (ou toi-même dans 6 mois) lit tes routes, il sait immédiatement que cette URL sert à détruire une ressource.  -->
                            @csrf
                            @method('DELETE')
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between p-3 bg-slate-50 rounded-xl">
                        ...
                        <span class="font-bold text-indigo-600">{{$questions->count()}}</span>
                    </div>
                    <div class="flex justify-between p-3 bg-slate-50 rounded-xl">
                        <span class="text-sm text-slate-500">Réponses ce jour</span>
                        <span class="font-bold text-indigo-600">{{$totalResponses}}</span>
                    </div>
                </div>
            </div>
        </aside>
